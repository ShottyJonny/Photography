...
- **Corrections (spec §11) are binding:** native-aspect Product plate + orientation from `aspect_ratio`; Product as one client-state island; real cart; collections visibility gate; Home empty state; kicker/ordering rules; ISR-vs-CI build env; seed via service key; `<picture>` lazy/LCP/null-dim.
